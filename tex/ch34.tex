...
\begin{enumerate}
\item \textit{How does IAC affect the number of visits to look up information from multiple sources with different IACs?}
\item \textit{How does IAC affect the sequence/order of visits to look up information from multiple sources with different IACs?}
\item \textit{Do the number and sequence of visits affect data entry accuracy and speed?}
\end{enumerate]
...
\subsubsection{Task}
The experimental data entry task was framed around an expenses task as observed in Study 1 and Study 2. In an original expenses task, the user has to complete a number of data entries regarding incurred expenses in order to claim back the money, such as the expense made, the reason for making the expense, the date of the expense, and the account on which to claim back the money. They fill this into a claim form, which looks similar to a spreadsheet. %where each row corresponds to one expense.

Because the focus of this study was on the time cost to retrieve the data to be entered, and not on different type of data items per se, the participants only had to fill in two type of data items. In the current study, participants had to fill in a financial amount to specify the expense that was made, such as  $\pounds$11.49, and an account code to determine from which account to claim back the expense, such as 654273. Participants had to enter two expenses per trial, and thus had to enter two amounts and two account codes.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \label{fig:mouse}
    \end{subfigure}
    \caption{Participants had to enter four data items in a data entry form (a). Each item had to be retrieved from a separate window (b) and entered into the form (c).}\label{fig:animals}
\end{figure}
