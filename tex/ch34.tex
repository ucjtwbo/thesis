...
\begin{table}
\centering
\includegraphics[width=\textwidth]{images/ch34/ch34-3_Means.pdf}
...
The boxplots in Figure \ref{fig:ch34_4-boxplots} show the variability of interleaving rates across conditions. The Low condition had a median interleaving rate of 6\%, the High-AM condition had a median interleaving rate of 12\%, and the High-AC condition had a median interleaving rate of 96\%.
...
Participants interleaved most often between expenses in the High-AC condition (\textit{M} = 73.2\%, \textit{SD} = 41.1\%), compared to the Low (\textit{M} = 31.17\%, \textit{SD} = 42.24\%) and High-AM (\textit{M} = 34.18\%, \textit{SD} = 41.5\% ) conditions, $\chi^2$(2) = 6.81, \textit{p} = 0.03. A post-hoc Dunn's test showed there was a difference between the High-AC condition and the Low (p = 0.02) and the High-AM (\textit{p} = 0.03) conditions, but not between the Low and High-AM conditions (\textit{p}=0.9).
...
The boxplots in Figure \ref{fig:ch34_5-boxplots} show the variability of interleaving rates across conditions. Participants interleaved most often between data entry sheets in the High-AC (\textit{M} = 73.4\%, \textit{SD} = 32.1\%) and High-AM (\textit{M} = 83.8\%, \textit{SD} = 21.6\% ) conditions compared to the Low (\textit{M} = 30.5\%, \textit{SD} = 37.7\%) condition, $\chi^2$(2) = 11.13, \textit{p} < 0.01. A post-hoc comparison showed there was a difference between the Low and the High-AM (\textit{p} < 0.01) and High-AC (\textit{p} = 0.01) conditions, and no difference between the High-AC condition and the High-AM (\textit{p} = 0.4) conditions.
...
As in Study 4, participants made on average four visits per trial, i.e. one visit per data entry. There was no difference in the number of visits, $\chi^2$(2) = 1.59, p = 0.5. Participants made significantly shorter visits in the Low (\textit{M} = 2.00s, \textit{SD} = 0.68s) condition compared to the High-AC condition (\textit{M} = 2.25s, \textit{SD} = 0.67s) compared to the High-AM (\textit{M} = 2.61s, \textit{SD} = 0.85s) and  $\chi^2$(2) = 6.14, \textit{p}= 0.04. Post-hoc comparisons found a significant difference between  the High-AM and the Low (\textit{p}=.02) conditions, but not between High-AC and Low conditions (\textit{p} = 0.2) or the High-AC and the High-AM (\textit{p} = 0.2).
...
When comparing across conditions, these types of errors happened on a significantly higher proportion of data entries in the High-AC (\textit{M} = 4.58\%, \textit{SD} = 3.6\%) and High-AM (\textit{M} = 6.54\%, \textit{SD} = 5.01\%) compared with the Low condition (\textit{M} = 1.23\%, \textit{SD} = 1.82\%),  $\chi^2$(2) = 11.29, \textit{p} < 0.01.  A post-hoc comparison showed there was a difference between the Low and the High-AM (\textit{p} < 0.01) and High-AC (\textit{p} = 0.01) conditions, and no difference between the High-AC condition and the High-AM (\textit{p} = 0.4) conditions.
